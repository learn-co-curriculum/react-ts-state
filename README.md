...
You may be wondering - when should you explicitly type state rather than rely on
type inference? Typically, if we provide a default state value, it is fine to
let TypeScript infer the type. When we do _not_ provide a default state value,
we need to explicitly provide a type.
...
> Don’t call Hooks from regular JavaScript or TypeScript functions.
